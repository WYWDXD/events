...
import org.example.bigevent.mapper.JianAnMapper;
...
@Service
public class JianAnServiceImpl implements JianAnService {
    @Autowired
    private JianAnMapper jianAnMapper;

    @Override
    public PageBean<JianAn> list(Integer pageNum, Integer pageSize, String code) {
        //创建pagebean对象
        PageBean<JianAn> pb = new PageBean<>();
        //开启分页查询    PageHelper
        PageHelper.startPage(pageNum, pageSize);

        List<JianAn> as = jianAnMapper.list(code);
        //page中提供了方法，可以获取pagehelper分页查询后得到的总记录条数和当前页数数据
        Page<JianAn> p = (Page<JianAn>) as;
        //把数据填充到pagebean对象中

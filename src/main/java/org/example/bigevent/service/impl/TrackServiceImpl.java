...
import org.example.bigevent.mapper.TrackMapper;
...
public class TrackServiceImpl implements TrackService {
    ...
    @Override
    public PageBean<Track> list(Integer pageNum, Integer pageSize, String code) {
        //创建pagebean对象
        PageBean<Track> pb = new PageBean<>();
        //开启分页查询    PageHelper
        PageHelper.startPage(pageNum, pageSize);

        List<Track> as = trackMapper.list(code);
        //page中提供了方法，可以获取pagehelper分页查询后得到的总记录条数和当前页数数据
        Page<Track> p = (Page<Track>) as;
        //把数据填充到pagebean对象中
